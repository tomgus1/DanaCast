...
public class Seriesblanco {
    public static ArrayList<EntryModel> getSearchResults(final String query) {
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    Document document = Jsoup.connect("http://seriesblanco.com/search.php" + "?q1=" + query.replace(" ", "+"))
                            .get();
                    Elements elements = document.getElementsByClass("post-header").first().getElementsByTag("a");
            ...
            public void run() {
                try {
                    Document document = Jsoup.connect("http://www.seriesblanco.com/")
                            .get();
                    Elements elements = document.select("#PopularPosts1").first().getElementsByTag("li");
                    for (Element element : elements) {
